...
export function Chat() {
    ...
    //                     break;
    //                 case 'chartPie':
    //                     setMessagesData((prev : Array<messageDataProps>) => [...prev, {position: 'left', type: 'chartPie', content: content}]);
    //                     setMessagesData((prev : Array<messageDataProps>) => [...prev, {position: 'left', type: 'text', content: {message: JSON.parse(message.body).content.message}}]);
    //                     break;
    //                 case 'chartLine':
    //                     setMessagesData((prev : Array<messageDataProps>) => [...prev, {position: 'left', type: 'chartLine', content: content}]);
    //                     setMessagesData((prev : Array<messageDataProps>) => [...prev, {position: 'left', type: 'text', content: {message: JSON.parse(message.body).content.message}}]);
    ...
    useEffect(() => {
        // setMessagesData((prev : Array<messageDataProps>) => [...prev, {position: 'left', type: 'button', content: {header: 'Отправка денег ', buttonBody : 'Перейти', url: 'https://music.yandex.ru/album/9046986/track/609676'}}]);
    ...
    return (
        ...
        <div className="relative flex flex-col justify-between p-8 h-full w-full border rounded-md border-input bg-background mob:p-1 mob:border-transparent">
            ...
            <ScrollArea className="relative w-full h-full p-3 mob:p-0">
                {messagesData.map((messageData, index) => (
                    ...
                    <AnimatePresence key={index} initial={true}>
                        ...
                        <div ref={lastMessageRef} className="mt-3 pr-3 mob:mt-5 mb-2 w-full overflow-hidden">
            ...
            <div className="relative top-auto w-full bottom-0">
